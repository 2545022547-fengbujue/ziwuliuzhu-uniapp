...
```powershell
powershell -Command "Start-Process -FilePath '系统目录\nodejs\node.exe' -ArgumentList 'node_modules\vite\bin/vite.js','--host','0.0.0.0' -WorkingDirectory '项目目录\ziwuliuzhu-uniapp' -WindowStyle Hidden"
```
...
# 检查局域网（手机访问用这个）
curl -s -o /dev/null -w "%{http_code}" http://内网IP:5174/
```
...
3. 当前电脑 IP：`内网IP`（手机热点分配，可能变化）
4. 手机浏览器访问：`http://内网IP:5174/`
...
使用 开发工具 CLI 云端打包，无需本地安装 Android SDK。

### 10.2 打包命令

```bash
"项目目录/开发工具/cli" pack \
  --project "项目目录/ziwuliuzhu-uniapp" \
  --platform android \
...
3. 项目必须先通过 `cli project open` 导入 开发工具
4. 开发工具 主程序必须在运行中

### 10.4 APK 输出位置

打包完成后 APK 下载到：`系统目录\下载目录\子午流注取穴.apk`
