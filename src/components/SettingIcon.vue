...
const pngSrc = computed(() => {
  ...
  // 经典四色（activeUiStyle='classic'）无专属 PNG——设置页文字前图标不做主题色适配，
  // 直接复用 modern 一份图（与 H5 端"各主题同一图标"的表现一致）。
  const iconKey = props.name === 'about' ? 'methods' : props.name
  const styleKey = store.activeUiStyle === 'classic' ? 'modern' : store.activeUiStyle
  return pngMap[iconKey]?.[styleKey] || ''
  // #endif
  // #ifndef APP-PLUS
  return ''
  // #endif
