...
export const useAppStore = defineStore('app', () => {
  ...
  function applyThemeChrome() {
    ...
    if (!chrome) return
    // 注意：uni.setTabBarStyle/setTabBarItem 在 uni-app x 中返回 Promise，
    // 非 tabBar 页面（如 web 首屏）会 reject —— Promise 拒绝不属于 try-catch 范畴，
    // 必须显式 .catch 抑制，否则抛 Uncaught (in promise) 中断初始化。
    const safe = (p) => (p && typeof p.catch === 'function' ? p.catch(() => {}) : p)
    try {
      const {
      ...
      // uni-app x 的 App 端不支持 uni.setTabBarStyle（仅 Web/小程序支持），
      // App 端仅切换 tabBar 图标，颜色降级为默认。
      // #ifndef APP
      safe(uni.setTabBarStyle(style))
      // #endif
      safe(uni.setTabBarItem({
        index: 0,
