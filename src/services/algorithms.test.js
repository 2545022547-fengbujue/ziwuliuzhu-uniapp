...
describe('纳甲法时辰干支与徐凤歌诀全量对照（10日×6时=60处）', () => {
  ...
  it('跨日顺推自洽：甲日周期开井(甲戌)后荥穴时辰为丙子——丙子在干支纪时上属于乙日/庚日的子时（乙庚丙作初），值日周期仍为甲日', () => {
    ...
    // 甲日周期从甲戌起顺推 12 时辰完整序列：甲戌(甲日)乙亥(甲日)丙子(乙日)丁丑(乙日)…
    // 干支纪时归属：甲戌/乙亥 属甲日自然日，丙子及之后属乙日自然日，但值日周期全程为甲日
    const stems = ['甲', '乙', '丙', '丁', '戊', '己', '庚', '辛', '壬', '癸', '甲', '乙']
    const branches = ['戌', '亥', '子', '丑', '寅', '卯', '辰', '巳', '午', '未', '申', '酉']
    stems.forEach((s, i) => {
      expect(r.dailySequence[(10 + i) % 12].hourStem, `甲日周期顺推+${i} 天干`).toBe(s)
      expect(r.dailySequence[(10 + i) % 12].hourBranch, `甲日周期顺推+${i} 地支`).toBe(branches[i])
    })
  })
})
...
/**
 * ============================================================
 * 五鼠遁（日上起时）约束（2026-08-14 用户指正后固化）
 *
 * 用户观点（正确）：甲日（自然日）不会出现丙子时。五鼠遁"甲己还加甲"，
 * 甲日子时天干固定为甲；丙子时只会出现在乙日、庚日的子时（乙庚丙作初）。
 *
 * 这与徐凤歌诀"甲日戌时胆窍阴，丙子时中前谷荣"并不矛盾：
 * 歌诀的"甲日"指「甲日值日周期」（覆盖甲日甲戌→乙日甲申，跨两自然日），
 * 而"丙子"是乙日（自然日）的子时——干支纪时与值日周期是两个正交维度。
 *
 * 本测试用真实 getGanZhi 验证干支纪时严格遵循五鼠遁：
 * 甲日全天绝无丙子；丙子仅属乙日/庚日子时。防止未来有人误解歌诀、
 * 或改动 getGanZhi 时破坏五鼠遁约束。
 * ============================================================
 */
describe('五鼠遁约束：丙子仅属乙/庚日子时，甲日（自然日）绝无丙子', () => {
  it('2026-05-20（甲日）全天 12 时辰干支 = 甲子…乙亥，无丙子', () => {
    const expected = ['甲子', '乙丑', '丙寅', '丁卯', '戊辰', '己巳', '庚午', '辛未', '壬申', '癸酉', '甲戌', '乙亥']
    for (let h = 0; h < 12; h++) {
      const d = new Date(2026, 4, 20, h * 2, 30) // 甲日（2026-05-20）
      const gz = getGanZhi(d)
      expect(gz.day.heavenlyStem, `h${h} 日干`).toBe('甲')
      expect(gz.hour.ganZhi, `h${h} 时干支`).toBe(expected[h])
      expect(gz.hour.ganZhi).not.toBe('丙子')
    }
  })
  ...
  it('五鼠遁 10 日子时全表：甲己→甲子、乙庚→丙子、丙辛→戊子、丁壬→庚子、戊癸→壬子', () => {
    // 五鼠遁歌诀：甲己还加甲，乙庚丙作初，丙辛从戊起，丁壬庚子居，戊癸何方发，壬子是真途
    const wushudun = { '甲': '甲子', '乙': '丙子', '丙': '戊子', '丁': '庚子', '戊': '壬子', '己': '甲子', '庚': '丙子', '辛': '戊子', '壬': '庚子', '癸': '壬子' }
    // 2026-05 逐日扫描，为每个日干找到真实样例日期（子时 0:30）
    for (const [dayStem, expectSub] of Object.entries(wushudun)) {
      let found = null
      for (let day = 1; day <= 31; day++) {
        const d = new Date(2026, 4, day, 0, 30)
        if (getGanZhi(d).day.heavenlyStem === dayStem) { found = d; break }
      }
      expect(found, `${dayStem}日样例`).toBeTruthy()
      const gz = getGanZhi(found)
      expect(gz.hour.ganZhi, `${dayStem}日子时（五鼠遁 ${expectSub}）`).toBe(expectSub)
    }
  })
})
