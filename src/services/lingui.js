...
const DAY_BRANCH_VALUES = {
  ...
  '申': 9, '酉': 9,       // 申酉→9
  '寅': 8, '卯': 8,       // 寅卯→8
  // 表10-15 代数7 的地支：教材《针灸治疗学》表格印作「己亥午子」，但歌诀原文为
  // 「戊癸巳午七相宜，丙辛亥子亦七数」→ 实为「巳午亥子」四地支。
  // 注：「巳(sì)」与「己(jǐ)」形近，古籍排印/OCR 常混写（教材此处即如此）；「己」是天干
  // （逐日代数 10），不可能出现在地支列。代码按歌诀实现：巳/亥/子/午 均为 7。
  '巳': 7, '亥': 7,       // 巳亥→7（教材印「己亥」即「巳亥」）
  '子': 7, '午': 7        // 子午→7（教材印「午子」即「子午」）
}
 ...
 *
 * 表10-16 八法临时干支代数：
 *   代数9=甲己子午，代数8=乙庚丑未，代数7=丙辛寅申，
 *   代数6=丁壬卯酉，代数5=戊癸辰戌，代数4=巳亥
 */
const HOUR_STEM_VALUES = {
  '甲': 9, '己': 9,       // 甲己→9
  '乙': 8, '庚': 8,       // 乙庚→8
  '丙': 7, '辛': 7,       // 丙辛→7
  '丁': 6, '壬': 6,       // 丁壬→6（修正：原错误为7）
  '戊': 5, '癸': 5        // 戊癸→5（修正：原错误为6）
}

const HOUR_BRANCH_VALUES = {
  '子': 9, '午': 9,       // 子午→9
  '丑': 8, '未': 8,       // 丑未→8
  // 表10-16 代数7 的地支：教材《针灸治疗学》表格印作「寅甲」，但歌诀原文为
  // 「丙辛寅申七作数」→ 实为「寅申」；「甲」是天干（临时代数 9），不可能出现在地支列，
  // 属「甲/申」形近排印/OCR 混写（与表10-15 的「巳/己」同类问题）。
  '寅': 7, '申': 7,       // 寅申→7（教材印「寅甲」即「寅申」）
  '卯': 6, '酉': 6,       // 卯酉→6
  '辰': 5, '戌': 5,       // 辰戌→5
  '巳': 4, '亥': 4        // 巳亥→4
}
